/* need to migrate all functionality into the router once the 
   ...
   // data taken from API website to authenticate and get access for fetch requests
   // token expires 12hours after creation
   ...
   const apiWrapper = {
     ...
     getPlantDetails: async (req, res, next) => {
       ...
       plantId.forEach((plantName) => {
         fetch(`https://open.plantbook.io/api/v1/plant/detail/${plantName}/`, {
         method: 'GET',
         headers: { 'Authorization': `Bearer ${TOKEN}` }
       })
         .then((body)=> body.json())
